...
export type MazeGrid = number[][]; // open-wall bitmask per cell

/** Recursive backtracker — returns open-wall bitmask grid */
export function generateMaze(cols: number, rows: number): MazeGrid {
  const maze: MazeGrid = Array.from({ length: rows }, () => new Array(cols).fill(0));
